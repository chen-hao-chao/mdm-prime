...
class PixelEmbedding(nn.Module):
    def __init__(self, n_tokens: int, hidden_size: int, target_length: int=1):
        super().__init__()
...
class DiscreteUNetModel(nn.Module):
    ...
    def __post_init__(self):
        super().__init__()
        assert (x := self.model_channels * self.channel_mult[0]) % self.in_channels == 0, \
            f"Unet input dimensions must be divisible by the number of channels. Got {x} / {self.in_channels}"
        
        self.embedding_dim = (self.model_channels * self.channel_mult[0] // self.in_channels)
...
@dataclass(eq=False)
class DiscretePrimeUNetModel(nn.Module):
    vocab_size: int
    in_channels: int = 3
    model_channels: int = 128
    out_channels: int = 3
    ...
    def __post_init__(self):
        ...
        self.embedding_dim = (self.model_channels * self.channel_mult[0] // self.in_channels)
        self.pixel_embedding = PixelEmbedding(
            n_tokens=self.base, hidden_size=(self.embedding_dim // self.target_length), target_length=self.target_length,
        )
    ...
    def forward(
        ...
        B, c, H, W = x_t.shape
        L = c // self.target_length
        emb = self.pixel_embedding(x_t)
        logits = self.unet(emb, t, extra)
        logits = logits.reshape(B, L, self.vocab_size, H, W).permute(0, 1, 3, 4, 2)
        return logits