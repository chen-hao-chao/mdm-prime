...
from utils.logging import print_nans
from utils.eval_utils import compute_mauve
...
def _sample_categorical(categorical_probs, dtype=torch.float32):
  ...
  gumbel_noise = -((torch.rand_like(categorical_probs, dtype=dtype) + eps).log())
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    if config.prime.target_length != 1:
      ...
      with torch.no_grad():
        extended_masks = create_extended_p_filter(lambda x, seq: encoder(x, base, target_length, seq),
                                                  ...
                                                  l=config.prime.target_length, 
                                                  b=config.prime.base)
      if self.carry_over:
        self.marginalization_fn = lambda logit, subtokens: convert_to_marginal_filter_logit(logit=logit, extended_masks=extended_masks, class_index=subtokens)
      else:
    ...
    if (not hasattr(self.tokenizer, 'mask_token')
      ...
      if self.config.prime.target_length == 1:
        self.vocab_size += 1
      print('masked token is NOT defined in tokenizer')
    else:
      self.mask_index = self.tokenizer.mask_token_id
      print('masked token is defined in tokenizer')

    self.mask_index_enc = self.config.prime.base
    self.parameterization = self.config.parameterization

    if self.config.backbone == 'dit' and self.config.prime.target_length == 1:
      self.backbone = models.dit.DIT(self.config, vocab_size=self.vocab_size)
    elif self.config.backbone == 'dit' and self.config.prime.target_length != 1:
      ...
      if self.config.prime.target_length == 1:
        self.backbone = models.autoregressive.AR(self.config,
                                                 vocab_size=self.vocab_size,
                                                 mask_index=self.mask_index)
      else:
        self.backbone = models.autoregressive.AR(self.config,
                                                 vocab_size=config.prime.base+1,
                                                 mask_index=config.prime.base)
    ...
    if config.eval.compute_zero_shot:
      self.eval_list = ['owt', 'lambada', 'wikitext2', 'ptb', 'lm1b', 'ag_news', 'pubmed', 'arxiv']
      self.zeroshot_metrics = torch.nn.ModuleDict({
          f"zeroshot/{self.eval_list[i]}": metrics.clone(prefix=f"zeroshot/{self.eval_list[i]}/")
    ...
    self.eval_model_tokenizer =\
      transformers.AutoTokenizer.from_pretrained(self.gen_ppl_eval_model_name_or_path)
  ...
  def _subs_prime_parameterization(self, logits, xt):
    if self.carry_over:
      log_p = self.marginalization_fn(logits, xt)
    else:
  ...
  def forward(self, x, sigma):
    """Returns log score."""
    sigma = self._process_sigma(sigma) # = 0 for not time-conditional model
    with torch.cuda.amp.autocast(dtype=torch.float32):
      logits = self.backbone(x, sigma)
    
    if self.parameterization == 'subs' and self.config.prime.target_length == 1:
      return self._subs_parameterization(logits=logits, xt=x)
    elif self.parameterization == 'subs' and self.config.prime.target_length != 1:
      return self._subs_prime_parameterization(logits=logits, xt=x)
  ...
  def _compute_loss(self, batch, prefix):
    ...
    elif prefix.split('/')[0] == 'zeroshot' and self.config.eval.compute_zero_shot:
      self.zeroshot_metrics[prefix].update(losses.nlls, losses.token_mask)
    else:
  ...
  def on_validation_epoch_start(self):
    ...
    if self.config.eval.compute_zero_shot:
      for dataset_name, metric_collection in self.zeroshot_metrics.items():
        assert metric_collection["nll"].mean_value == 0, f"{dataset_name} NLL mean_value is not zero"
        assert metric_collection["nll"].weight == 0, f"{dataset_name} NLL weight is not zero"
  ...
  def validation_step(self, batch, batch_idx, dataloader_idx=0):
    if dataloader_idx == 0:
      ...
      loss = self._compute_loss(batch, prefix='zeroshot/{}'.format(self.eval_list[dataloader_idx]))
    return loss

  def on_validation_epoch_end(self):
    if self.config.eval.compute_zero_shot:
      log_dict = {}
      for key, metric in self.zeroshot_metrics.items():
          log_dict.update(metric.compute())  # Collect all computed values
      for metric_collection in self.zeroshot_metrics.values():
    ...
    if self.config.eval.conditional_sampling:
      text_list = []
      gt_text_list = []
      iterator = iter(self.test_eval_loader)
      for _ in range(self.config.sampling.num_sample_batches):
        conditions_load = next(iterator)['input_ids']
        conditions = conditions_load.to(self.device)
        samples = self._sample(conditions=conditions)
        # Decode the samples to be re-tokenized by eval model
        text_samples = self.tokenizer.batch_decode(samples)
        text_list += text_samples
        gt_text_samples = self.tokenizer.batch_decode(conditions_load)
        gt_text_list += gt_text_samples
      mauve = compute_mauve(text_list, gt_text_list)
      self.log('val/mauve', mauve.mauve,
                on_epoch=True,
                on_step=False,
                sync_dist=True)
      ...
      if self.trainer.global_rank == 0 and hasattr(
          ...
          text_samples = text_samples[
            : self.config.sampling.num_sample_log]
          self.trainer.logger.log_table(
            key=f'conditional_samples@global_step{self.global_step}',
            columns=['Generated Conditional Samples'],
            data=[[s] for s in text_samples])
  ...
  def q_xt(self, x, move_chance):
    ...
    if self.config.prime.target_length == 1:
      ...
      xt = torch.where(move_indices, self.mask_index_enc, x)
    return xt
  
  @torch.no_grad()
  def _nucleus_sample(self, p_x0):
    # source: https://github.com/kuleshov-group/bd3lms/blob/main/diffusion.py
  ...
  def _sample_prior(self, *batch_dims):
    if self.config.prime.target_length == 1:
      ...
      return self.mask_index_enc * torch.ones(
        * batch_dims, dtype=torch.int64)

  def _ddpm_caching_update(self, x, t, dt, p_x0=None, return_x0=False):
  ...
  def _ar_sampler(self, bsz, conditions=None):
    ...
    if conditions is not None:
      ...
      if self.config.prime.target_length != 1:
        conditions = self.wrapped_encoder(conditions)
      x[:, : self.config.sampling.prefix_len * self.config.prime.target_length] = conditions[:, : self.config.sampling.prefix_len * self.config.prime.target_length]
      indent = self.config.sampling.prefix_len * self.config.prime.target_length
    else:
      x[:, 0] = self.tokenizer.bos_token_id
      indent = 1
    
    num_pred_tokens = self.config.model.length - indent
    for i in range(num_pred_tokens):
      p_x0 = torch.exp(self.forward(x[:, :i + indent], None)[:, -1])
      p_x0 = self._nucleus_sample(p_x0)
      y = _sample_categorical(p_x0, dtype=self.sampling_dtype)
      x[:, i + indent] = y
  ...
  @torch.no_grad()
  def _sample(self, num_steps=None, eps=1e-5, conditions=None):
    """Generate samples from the model."""
    batch_size_per_gpu = self.config.loader.eval_batch_size
    if self.parameterization == 'ar':
      return self._ar_sampler(batch_size_per_gpu, conditions)
    ...
    x = self._sample_prior(
      batch_size_per_gpu,
      self.config.model.length * self.config.prime.target_length).to(self.device)
    
    if conditions is not None:
      conditions = conditions.to(self.device)
      if self.config.prime.target_length != 1:
        conditions = self.wrapped_encoder(conditions)
      x[:, : self.config.sampling.prefix_len * self.config.prime.target_length] =\
          conditions[:, : self.config.sampling.prefix_len * self.config.prime.target_length] 
    ...
    for i in range(num_steps):
      print("Iterating {}-th step...".format(i))
      ...
      if self.sampler == 'ddpm':
        ...
        if self.config.prime.target_length == 1:
          ...
          p_x0_cache, x_next = self._mdm_prime_update(
            x, t, dt, p_x0=p_x0_cache)
        if (not torch.allclose(x_next, x)
            or self.time_conditioning): 
          # if input remains the same, then cache...
    ...
    if self.config.sampling.noise_removal:
      ...
      if self.sampler == 'analytic':
        ...
        if self.config.prime.target_length == 1:
          ...
          p = torch.exp(self.forward(x, unet_conditioning))
          _x = self.wrapped_encoder(_sample_categorical(p, dtype=self.sampling_dtype))
  ...
  def restore_model_and_sample(self, num_steps, eps=1e-5, conditions=None):
    """Generate samples from the model."""
    # Lightning auto-casting is not working in this method for some reason
    if self.ema:
      self.ema.store(itertools.chain(
  ...
  def _analytic_update(self, x, t, step_size):
    ...
    return _sample_categorical(probs, dtype=self.sampling_dtype)
  ...
  def _denoiser_update(self, x, t):
    ...
    samples = _sample_categorical(probs, dtype=self.sampling_dtype)
    return samples
  ...
  def _forward_pass_diffusion(self, x0):
    ...
    print_nans(model_output, 'model_output')
    ...
    try:
      assert x0.max() < self.vocab_size, "Target label exceeds number of classes"
    except AssertionError as e:
      print(f"[Warning] Skipping batch due to: {e} || Catch maximum value {x0.max()}")
  ...
  def _loss(self, x0, attention_mask):
    if self.parameterization == 'ar':
      if self.config.prime.target_length != 1:
        x0 = self.wrapped_encoder(x0)
        attention_mask = attention_mask.repeat_interleave(self.config.prime.target_length, dim=1)
  ...
  # monitor zero-shot ppl
  @torch.no_grad
  def calculate_zero_shot_ppl(self):
    return 0
