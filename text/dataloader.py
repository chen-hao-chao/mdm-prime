...
from utils.file_path import fsspec_exists, fsspec_mkdirs
...
def get_dataset(
  ...
  if dataset_name == 'wikitext103':
    ...
    dataset = datasets.load_dataset(
      ...
      storage_options={'client_kwargs': {'timeout': aiohttp.ClientTimeout(total=12000)}})
  elif dataset_name == 'lambada':
    dataset = get_lambada_test_dataset()
  elif dataset_name == 'text8':
    assert wrap
    dataset = get_text8_dataset(
      cache_dir=cache_dir, max_seq_length=block_size)
  ...
  if dataset_name in ['lambada', 'openwebtext-train',
                      'openwebtext-valid', 'openwebtext-test']:
    data = dataset
  else:
    data = dataset[mode]
...
def get_dataloaders(config, tokenizer, dataset_name, skip_train=False,
                    skip_valid=False, skip_test=True, valid_seed=None):
